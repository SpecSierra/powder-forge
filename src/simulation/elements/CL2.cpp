...
static int update(UPDATE_FUNC_ARGS)
{
	...
	for (auto ry = -1; ry <= 1; ry++)
	{
		...
		if ((TYP(r) == PT_WATR || TYP(r) == PT_DSTW) && sim->rng.chance(1, 50))
		{
		...
		if (TYP(r) == PT_H2 && sim->rng.chance(1, 50))
		{
		...
		if (TYP(r) == PT_IRON && sim->rng.chance(1, 100))
		{
		...
		if (TYP(r) == PT_NH3 && sim->rng.chance(1, 50))
		{
